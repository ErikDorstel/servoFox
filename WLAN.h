...
const char* ssidAP="servoFox"; const char* passwordAP="";
String ssidStation=""; String passwordStation=""; boolean statusStation=false;

void WiFiStationConnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  preferences.begin("servoFox",false); preferences.putString("ssidStation",ssidStation); preferences.putString("passwordStation",passwordStation); preferences.end();
...
void WiFiStationDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  preferences.begin("servoFox",false); String ssidStationOld=preferences.getString("ssidStation",""); String passwordStationOld=preferences.getString("passwordStation",""); preferences.end();
  if (ssidStationOld==ssidStation & passwordStationOld==passwordStation & WiFi.softAPgetStationNum()==0) { WiFi.reconnect(); } }

void initWLAN() {
  preferences.begin("servoFox",false); ssidStation=preferences.getString("ssidStation",""); passwordStation=preferences.getString("passwordStation",""); preferences.end();
  ...
  WiFi.setHostname("servoFox"); WiFi.begin(ssidStation.c_str(),passwordStation.c_str());
