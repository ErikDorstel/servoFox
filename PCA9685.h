...
void setServo(int servoFreq,int servoValue) {
  pwm.setPWMFreq(servoFreq);
  for (int servo=0;servo<=15;servo++) { pwm.setPWM(servo,0,servoValue); }
  if (debug) { Serial.println("Servo Value " + String(servoValue) + " at " + String(servoFreq) + " Hz written."); } }
